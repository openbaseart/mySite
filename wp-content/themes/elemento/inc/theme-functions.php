...
if( ! function_exists( 'elemento_banner_slider' ) ):

	function elemento_banner_slider(){

		$slides = elemento_slider_data();
		?>
		<div class="banner-slider-wrap relative">
			<div class="flexslider top_slider" data-speed="<?php echo elemento_slider_speed();?>" data-animation="<?php echo elemento_slider_animation_speed();?>">
				<ul class="slides">
					<?php 
					foreach( $slides as $slide):
						if( $slide['image']):
							...
							<li class="slide1" style="background-image: url('<?php echo esc_url($slide['image']); ?>');background-position: center;">
								<div class="<?php echo elemento_site_container();?>  banner-caption">
									<div class="flex_caption1">
										<?php echo apply_filters('elemento_slide_title', $slide['title'], $slide['control'] );?>
										<?php echo apply_filters('elemento_slide_subtitle', $slide['subtitle'] , $slide['control'] );?>						
										<?php 
										$scroll_class= '';
										if( $slide['label'] ):
...
if( ! function_exists('elemento_slider_data') ):

	function elemento_slider_data(){
		...
		if( ! $title1 && ! $slide1 ){
			$title1 =  __('Welcome to Elemento','elemento');
		}
		if( ! $subtitle1 && ! $slide1 ){
			$subtitle1 =  __('Drag & Drop Theme','elemento');
		}

		if( ! $label1  && ! $slide1 ){
			$label1 = __('Click here','elemento');
		}
		if( ! $url1  && ! $slide1 ){
			$url1 =  '#about';
		}
		...
		if( ! $title2 && ! $slide2 ){
			$title2 = __('Ready to explore great theme?','elemento');
		}
		if( ! $subtitle2  && ! $slide2 ){
			$subtitle2 = __('Feel free to go around','elemento');
		}

		if( ! $slide2 ){
			$slide2 = esc_url(get_template_directory_uri().'/assets/images/slider1.jpg');
		}
...
function elemento_dynamic_css(){
	...
	<style>
		...
		.tagcloud .tag-cloud-link{
			border: 2px solid <?php echo get_theme_mod('theme_border_color'); ?> !important;
            }
